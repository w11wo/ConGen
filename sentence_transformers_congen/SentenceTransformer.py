...
class SentenceTransformer(nn.Sequential):
    ...
    def smart_batching_collate(self, batch):
        ...
        try:
            ...
            for example in batch:
                ...
                sents1_encoded.append(example.label)
            sents1_encoded = torch.stack(sents1_encoded)
            ...
            sents1_features = sentence_features[0]
        ...
        for epoch in trange(epochs, desc="Epoch", disable=not show_progress_bar):
            ...
            for _ in trange(steps_per_epoch, desc="Iteration", smoothing=0.05, disable=not show_progress_bar):
                for train_idx in range(num_train_objectives):
                    ...
                    if str(type(loss_model)) == "<class 'sentence_transformers_congen.losses.ConGenLoss.ConGenLoss'>":
                        sents1_features, sents2_features, sents1_encoded = data
                    else:
                    ...
                    if use_amp:
                        with autocast():
                            if str(type(loss_model)) ==  "<class 'sentence_transformers_congen.losses.ConGenLoss.ConGenLoss'>":
                                loss_value = loss_model(sents1_features,
                                                        sents2_features,
                                                        sents1_encoded)
                            else:
    ...
    def _eval_during_training(self, evaluator, output_path, save_best_model, epoch, steps, callback, end_of_epoch=False):
        ...
        if evaluator is not None:
            ...
            if end_of_epoch and self.early_stopping_patience_counter == self.early_stopping_patience:
                print(f'Model training will be early stopped due to the validation metric did not improve for {self.early_stopping_patience} epochs')
